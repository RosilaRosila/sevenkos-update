...
class User_model extends CI_Model
{
	...
	function edit_data_kota($where, $table)
	{
		return $this->db->get_where($table, $where);
	}

	function update_data_kota($where, $data, $table)
	{
		$this->db->where($where);
		$this->db->update($table, $data);
	}
	...
	public function checkDuplicate($value)
	{
		...
		$count = $this->db->count_all_results('kota');
		return $count;
	}

	function update_logo($navtext, $NavbarIcon)
	{
		return $this->db->query("UPDATE `setting` SET `primary` = '$navtext', `secondary` = '$NavbarIcon' WHERE `setting`.`id_logo` = 1;");
	}
}